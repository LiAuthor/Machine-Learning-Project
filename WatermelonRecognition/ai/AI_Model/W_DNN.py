...
def initialize_parameters_deep(layers_dims):
    ...
    for i in range(1, L):
        parameters["W"+str(i)] = np.random.randn(layers_dims[i],
                                                 layers_dims[i-1])/np.sqrt(layers_dims[i-1])
...
def L_layer_model(X, Y, layers_dims, learning_rate=0.035, num_iterations=5000, print_cost=False, isPlot=True):
    """
    ...
    if isPlot:
        ...
        plt.show()
    # 保存参数
    with open('W_parameters.pkl', 'wb') as f:
